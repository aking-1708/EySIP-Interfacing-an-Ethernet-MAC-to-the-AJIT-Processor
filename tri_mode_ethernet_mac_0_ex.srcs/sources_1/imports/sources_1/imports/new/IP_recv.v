	...
	if (reset) begin
		...
		case (cnt) 
			0: begin
				...
				if (ip_data[`HLEN] == 4'd5) begin  //In an IPv4 packet value of HLEN is 5
					options <= 1'b0;
				end
				else if (ip_data[`HLEN] > 4'd5) begin // if HLEN >5 then include options
					options <= 1'b1;
				end
				cnt <= cnt + 16'b1;
				bufcnt <= 1'b1;
			...
			$display("outputing: %h i have %d words left", ip_data_buffer, data_length);
				...
				udp_data <= ip_data_buffer;
				data_length <= data_length - 16'h0001;
				//cnt <= cnt + 16'b1;  // cnt is not incremented
			end
			default: cnt <= 16'h0;
		endcase
	end
