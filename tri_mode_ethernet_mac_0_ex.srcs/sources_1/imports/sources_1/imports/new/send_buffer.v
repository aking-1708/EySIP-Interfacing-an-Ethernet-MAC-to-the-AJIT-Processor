...
reg [7:0] ri_count;
reg ri_buf_wait; //goes high after receiving ip data, low after sent to axi
	...
	begin
		case (ri_state)
			...
			`WAIT: begin
				...
				if (ri_mac_addr != 48'h0) begin // If translation received then tell AXI interface to pick up data
					axi_ip_ready <= 1'b1;
				end
			end
		endcase
