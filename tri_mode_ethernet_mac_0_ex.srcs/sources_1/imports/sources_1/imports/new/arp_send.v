...
SPA (HardCded in Top Level Module)
SHA (HardCoded in Top Level Module)
       ...
       if (reset) begin
		...
		case ({request_en, reply_en})
			...
			begin
				...
				// If both request and replies are enaled simultaneously then address the request and buffer the reply.
				ip_reply_buffer <= send_ip_addr_reply;
				reply_buffer_valid <= 1'b1;
				mac_reply_buffer <= send_mac_addr_reply;
			end
		...
		//words using word_counter.
               	if (send_buffer_ready) begin
                    ...
                    word_counter <= 3'd0;
                               ...
                               default: //Dummy : Will Not reach this stage
                               begin
					arp_data <= 32'b0;
					arp_valid <= 1'b0;
                    word_counter <= 3'd0;
